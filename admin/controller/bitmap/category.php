<?php
class ControllerBitmapCategory extends Controller {
	private $error = array();
	...
	public function insert() {
		$this->load->language('bitmap/category');
		
		$this->load->model('bitmap/category');
		
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_bitmap_category->addCategory($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');
			
			$this->redirect($this->url->link('bitmap/category', 'token=' . $this->session->data['token'], 'SSL')); 
		}
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_bitmap_category->editCategory($this->request->get['id_category'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $id_category) {
				$this->model_bitmap_category->deleteCategory($id_category);
			}
	...
	private function getList() {
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('text_bitmap_administration'),
			'href'      => $this->url->link('bitmap/bitmap', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('bitmap/category', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		...
		$this->data['categories'] = $this->recursiveCategories($this->model_bitmap_category->getCategoriesByParentId());
		
		$this->data['delete'] = $this->url->link('bitmap/category/delete', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['add_category'] = $this->url->link('bitmap/category/insert', 'token=' . $this->session->data['token'] , 'SSL');
		...
		$this->template = 'bitmap/category_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function recursiveCategories($categories, $level=0) {
		...
		foreach ($categories as $category) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('bitmap/category/update', 'token=' . $this->session->data['token'] . '&id_category=' . $category['id_category'] , 'SSL')
			);		
			$indent = '';
			for($i=0; $i<$level; $i++) { 
				$indent .= "&nbsp;---&nbsp;"; 
	...
	private function getForm() {
		...
		if (!isset($this->request->get['id_category'])) {
			$this->data['action'] = $this->url->link('bitmap/category/insert', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['action'] = $this->url->link('bitmap/category/update', 'token=' . $this->session->data['token'] . '&id_category=' . $this->request->get['id_category'], 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('bitmap/category', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['token'] = $this->session->data['token'];

		if (isset($this->request->get['id_category']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$category_info = $this->model_bitmap_category->getCategory($this->request->get['id_category']);
    	}
		
		$categories = $this->recursiveCategories($this->model_bitmap_category->getCategoriesByParentId(), 1);
		...
		$this->template = 'bitmap/category_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm() {
		if (!$this->user->hasPermission('modify', 'bitmap/category')) {
	...
	private function validateDelete() {
  	...
  	public function category_tab() {
		...
		$this->load->language('bitmap/category_tab');
		...
		$this->template = 'bitmap/category_tab.tpl';
