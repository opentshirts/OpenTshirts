...
<table class="list" style="margin-bottom:0;">
  <thead>
	<tr>
	  <td width="50" style="text-align: center;"><input type="checkbox" onclick="$('input[name*=\'data[dtg_colors]\']').attr('checked', this.checked);" /></td>
	  ...
	  <td width="100" class="left"><?php echo $column_need_white_base; ?></td>
	...
	<?php } else { ?>
		...
		<tr>
			...
			<?php if (!$color['isdefault']) { ?>
				...
				<input type="checkbox" name="data[dtg_colors][]" value="<?php echo $color['id_design_color']; ?>" checked="checked" />
				<?php } else { ?>
				<input type="checkbox" name="data[dtg_colors][]" value="<?php echo $color['id_design_color']; ?>" />
				<?php } ?>
			<?php } ?>
			</td>
