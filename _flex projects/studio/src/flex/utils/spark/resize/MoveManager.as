package flex.utils.spark.resize {
	...
	public class MoveManager {
		...
		public function MoveManager(moveComponent:UIComponent = null, dragComponent:UIComponent = null) {
			dragging = false;
			_enabled = true;
			_bringToFrontOnMove = true;
			_constrainToParentBounds = true;
		...
		private function dragComponentMouseDown(event:MouseEvent):void {
			...
			if (bringToFrontOnMove && moveComponent.parent) {
				...
				if (index != last) {
					...
					var p:IVisualElementContainer = moveComponent.parent as IVisualElementContainer;
					p.setElementIndex(moveComponent, last);
				}
			}
